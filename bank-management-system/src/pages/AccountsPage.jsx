  ...
  if (error) {
    return (
      <div className="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative" role="alert">
        ...
        <span className="block sm:inline"> {error.message || 'An error occurred while fetching accounts.'}</span>
      </div>
    );
  }
        ...
        <table className="min-w-full bg-white shadow-md">
          ...
          <tbody>
            {accounts.map((account) => (
              <tr key={account.AccountNumber} className='hover:bg-gray-100 hover:cursor-pointer'>
